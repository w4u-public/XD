(function() {
	...
	var addForChildsModifier = function() {
		...
		forChildModifier.selectbox.querySelectorAll("option").forEach(function(option) {
			if(option.label == "ヾ(>▽<)ゞ") option.setAttribute("selected", "true");
		});
	...
	var setParameterSettings = function() {
		...
		addSelectboxFromParameterArray(parameterArray);
		changeApplyTypeFromParameter(parameterArray);
		...
		function addSelectboxFromParameterArray(array) {
			var param = array.concat();
			setFirstSelectBox(param.shift());
			param.map(function(value) {
				...
				} else if(isMixKey(value)) {
					var selectboxGroup = addSelectboxControllSet({
						target: controlStatus.mixSelections(),
						button: createElement({"tagName": "span", "text": "-", "attr": {"class": "controlField_key-remove"}}),
						optionData: controlStatus.keys(),
						className: ["key"]
					});
				...
				for(var i = 0; i < optionTags.length; i++) {
					var optionTag = optionTags[i];
					if(optionTag.textContent == value) {
						optionTag.selected = true;
						break;
					}
				}
			...
			function isModifier(param) {
				var modifiers = controlStatus.modifiers();
				for(key in modifiers) {
					if(modifiers[key].includes(param)) return param;
				}
			}

			function isMixKey(param) {
				var keys = controlStatus.keys();
				for(key in keys) {
					if(keys[key].includes(param)) return param;
				}
			}

			function setFirstSelectBox(firstKeyParam) {
				var selectbox = controlStatus.mainSelections().querySelectorAll("select.key")[0];
				var optionTags = toArray(selectbox.querySelectorAll("option"));
				for(var i = 0; i < optionTags.length; i++) {
					var optionTag = optionTags[i];
					if(optionTag.textContent == firstKeyParam) {
						optionTag.selected = true;
						break;
					}
				}
	...
	var getValidParameterArray = function() {
		...
		function filterValidValue(parameterArray) {
			...
			var decodeValidValueArray = validValueArray.map(function(encodeValue) {
				return decodeURI(encodeValue);
			})
			return decodeValidValueArray;

			function isValid(value) {
				var decodeValue = decodeURI(value);
				for(var label in definedKeys) {
					for(var i = 0; i < definedKeys[label].length; i++) {
						var text = definedKeys[label][i];
						if(text == decodeValue.toLowerCase()) return true;
					}
				}
